...
def test_multiplyRandNum(mocker):
    mocked_random = mocker.patch.object(calculator.random, "randrange", return_value=2)
    actual_result = calculator.multiplyRandNum(3)
    assert actual_result == 6
    mocked_random.assert_called_once_with(1, 100)
